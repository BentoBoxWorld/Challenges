...
Resources in `src/main/resources/`: `addon.yml` (addon metadata read by BentoBox), `config.yml` (user settings → `config/Settings`), `default.json` (bundled default challenges), `template.yml`, `panels/*.yml` (GUI layouts loaded by panelutils), `locales/*.yml` (translations — `en-US.yml` is the reference; the other locales are now generated by Claude, e.g. via the `/sync-locales` skill, with occasional corrections from admins. GitLocalize is no longer used).
